...
mod tests {
    ...
    #[test]
    fn update_proportion_computes_ratio() {
        let mut wp = WinProportion {
            wins: 3,
            total: 4,
            proportion: 0.0,
        };
        wp.update_proportion();
        assert!((wp.proportion - 0.75).abs() < 1e-6);
    }

    #[test]
    fn update_proportion_all_wins() {
        let mut wp = WinProportion {
            wins: 10,
            total: 10,
            proportion: 0.0,
        ...
        wp.update_proportion();
        assert!((wp.proportion - 1.0).abs() < 1e-6);
    }

    #[test]
    fn update_proportion_no_wins() {
        let mut wp = WinProportion {
            wins: 0,
            total: 7,
            proportion: 99.0, // stale value should be overwritten
        };
        wp.update_proportion();
        assert_eq!(wp.proportion, 0.0);
    }

    #[test]
    fn display_format_has_percent_sign() {
        let mut wp = WinProportion {
            wins: 1,
            total: 2,
            proportion: 0.0,
        ...
        wp.update_proportion();
        let rendered = format!("{wp}");
        assert!(rendered.contains('%'));
        assert!(rendered.contains("1 wins"));
